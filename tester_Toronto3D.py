...
class ModelTester:
    ...
    def test(self, model, dataset, gen_pseudo=None, num_votes=100):
        ...
        while last_min < num_votes:
            try:
                ...
                stacked_probs, stacked_labels, point_idx, cloud_idx, acc = self.sess.run(ops,
                ...
                if last_min + 1 < new_min:
                    ...
                    if int(np.ceil(new_min)) % 1 == 0:
                        ...
                        for i_test in range(num_val):
                            ...
                            invalid_idx = np.where(labels == model.config.ignored_label_inds)[0]
                            labels = np.delete(labels, invalid_idx)
